...
get_header();
?>
<main id="main-content" aria-labelledby="archive-title" class="layout__main layout__content new-archive">
  ...
  <div class="container">
    ...
    do_action( 'lambros_before_archive_loop' );
     ...
     if ( have_posts() ) : ?>
    <header class="archive-header site-header" aria-labelledby="archive-title">
      <?php
        if ( is_home() && ! is_front_page() ) {
          // Blog posts page set in Settings → Reading
          $posts_page_id = get_option( 'page_for_posts' );
          echo '<h1 id="archive-title">' . esc_html( get_the_title( $posts_page_id ) ) . '</h1>';
        } else {
          the_archive_title( '<h1 id="archive-title">', '</h1>' );
        }
        the_archive_description( '<p>', '</p>' );
      ?>
    </header>
      <section id="posts" class="section" aria-labelledby="archive-title">
      <?php
        while(have_posts()) :
          the_post();
